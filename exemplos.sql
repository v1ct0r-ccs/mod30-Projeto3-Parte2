...
create table tb_cliente (
	...
	idade bigint constraint check_idade CHECK (idade > 14 and idade < 120) not null,
...
CREATE OR REPLACE FUNCTION check_produto_quantidade()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  ...
  FROM tb_produto
  WHERE id = NEW.id_produto_fk;

  IF quantidade_estoque < NEW.quantidade THEN
    RAISE EXCEPTION 'Quantidade em estoque insuficiente, itens disponíveis atualmente: %', quantidade_estoque;
  END IF;
...
CREATE TRIGGER tg_produto_quantidade
BEFORE INSERT ON tb_produto_quantidade
FOR EACH ROW
EXECUTE FUNCTION check_produto_quantidade();
